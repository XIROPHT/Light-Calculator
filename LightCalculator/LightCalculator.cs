...
namespace LightCalculator
{
    ...
    public class LightCalculator : Game
    {
        ...
        private void CalculPositionGraphique()
        {
            ...
            int basePositionBouton = (GraphicsDevice.Viewport.Height * 35) / 100;
            int basePositionCalcul = (GraphicsDevice.Viewport.Height * 5) / 100;


            int basePositionResultat = (GraphicsDevice.Viewport.Height * 25) / 100;
            ...
            int hauteurBoutonAide = (GraphicsDevice.Viewport.Height * 5) / 100;

            _ecartResultatCalcul = basePositionResultat - basePositionCalcul;


            RectangleBoutonViderHistorique = new Rectangle(-5, ((GraphicsDevice.Viewport.Height * 90) / 100),
                GraphicsDevice.Viewport.Width + ((GraphicsDevice.Viewport.Width * 5) / 100),
                ((GraphicsDevice.Viewport.Height) * 10 / 100));

            RectangleBoutonRetourHistorique = new Rectangle(-5,
            ...
            RectangleBoutonInterrogation =
                new Rectangle((int) (GraphicsDevice.Viewport.Width - (largeurBoutonAide * 1.5f)), 0, largeurBoutonAide,
                    hauteurBoutonAide);

            int largeurBouton = (int) (GraphicsDevice.Viewport.Width * 25 / 100);
            int hauteurBouton = (int) (GraphicsDevice.Viewport.Height * 13 / 100);
            ...
            foreach (var t in listeBouton)
            {
                if (ListeRectangleBouton.Count > 0)
                {
                    ...
                    if (nouveauSaut)
                    {
                        ...
                        rectangleBouton =
                            new Rectangle(0,
                                ListeRectangleBouton[ListeRectangleBouton.Count - 1].Item2.Y + hauteurBouton,
                    ...
                    else
                    {
                        rectangleBouton =
                            new Rectangle(
                                largeurBouton + ListeRectangleBouton[ListeRectangleBouton.Count - 1].Item2.X,
                                ListeRectangleBouton[ListeRectangleBouton.Count - 1].Item2.Y, largeurBouton,
                                hauteurBouton);
                    }
                ...
                {
                    Rectangle rectangleBouton = new Rectangle(0, basePositionBouton, largeurBouton, hauteurBouton);
                    ListeRectangleBouton.Add(new Tuple<string, Rectangle>(t, rectangleBouton));
                }
            }
#if DEBUG
/* Console.WriteLine("Base position: " +
                   (ListeRectangleBouton[ListeRectangleBouton.Count - 1].Item2.Y +
                    ListeRectangleBouton[ListeRectangleBouton.Count - 1].Item2.Height) + "/" +
                   GraphicsDevice.Viewport.Width);*/
#endif
                        ...
                        if (_affichageHistorique == false)
                        {
                                ...
                                Task.Factory.StartNew(delegate()
                                    {
                                        try
                                        {
                                            ...
                                            try
                                            {
                                                using (var computeCalculation = new DataTable())
                                                {
                                                    testCalcul =
                                                        float.Parse(computeCalculation.Compute(tmpCalcul, null)
                                                            .ToString());
                                                    bonCalcul = true;
                                                }
                                            }
                            ...
                            switch (valeurBouton)
                            {
                                case "C":
                                    ...
                                    try
                                    {
                                        using (var computeCalculation = new DataTable())
                                        {
                                            float.Parse(computeCalculation.Compute(_calcul, null).ToString());
                                            bonCalcul = true;
                                        }
                                    }
                                    catch
                            ...
                            if (_calcul != null)
                            {
                                ...
                                try
                                {
                                    using (var computeCalculation = new DataTable())
                                    {
                                        tmpResultat = float.Parse(computeCalculation.Compute(_calcul, null).ToString());
                                        ok = true;
                                    }
                                }
                                catch
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (_affichageHistorique == false)
            {
                if (ListeRectangleBouton.Count > 0)
                {
                    ...
                    if (_resultat != 0)
                    {
                        spriteBatch.DrawString(_policeEcriture, $"{_resultat:0.#########}",
                            new Vector2(RectangleResultat.X, RectangleResultat.Y),
                            Color.Black);
                    }
                    else
                    {
                        spriteBatch.DrawString(_policeEcriture, _resultat.ToString(),
                            new Vector2(RectangleResultat.X, RectangleResultat.Y),
                            Color.Black);
                    }
            ...
            else // Affichage de l'historique
            {
                ...
                for (int i = 0; i < ListeHistoriqueCalcul.Count; i++)
                {
                    ...
                    {
                        float calcul =
                            float.Parse(computeCalculation.Compute(ListeHistoriqueCalcul[i].Item1, null).ToString());
                        spriteBatch.DrawString(_policeEcriture,
                            (i + 1) + " - " + ListeHistoriqueCalcul[i].Item2 + " = " + $"{calcul:0.#########}",
